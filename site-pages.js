(function () {
  ...
  function initCartPage() {
    ...
    var guestCheckoutAllowed = false;
    ...
    function confirmValidatedCheckout(targetStatus) {
      if (!connectedClient && !guestCheckoutAllowed) {
        openAuthModal();
        setStatus(targetStatus || validateStatus, "err", "Choisis connexion, inscription ou invite avant de valider le panier.");
        return false;
      }
    ...
    if ($("checkout-guest-btn")) $("checkout-guest-btn").addEventListener("click", function () {
      guestCheckoutAllowed = true;
      closeAuthModal();
      setStatus(validateStatus, "ok", "Mode invite selectionne. Renseigne simplement les coordonnees de commande.");
    });
    ...
    if ($("checkout-register-btn")) {
      $("checkout-register-btn").addEventListener("click", function () {
        ...
        fetch(API_BASE + "/api/client/register", {
          ...
          .then(function () {
            guestCheckoutAllowed = true;
            setStatus(authStatus, "ok", "Compte cree. Confirme ton email ensuite. Pour cette commande, tu peux continuer en invite.");
          })
          .catch(function (error) {
  ...
  function initClientPage() {
    ...
    var forgotView = $("client-forgot-block");
    var resetView = $("client-reset-block");

    function showAuthBlock(target) {
      [loginView, registerView, forgotView, resetView].forEach(function (view) {
    ...
    $("open-forgot").addEventListener("click", function () {
      showAuthBlock(forgotView);
    });
    $("back-login-from-register").addEventListener("click", function () {
      showAuthBlock(loginView);
    });
    $("back-login-from-forgot").addEventListener("click", function () {
      showAuthBlock(loginView);
    });
    ...
    $("magic-link-btn").addEventListener("click", function () {
      handleJsonForm("/api/client/login", {
        email: $("login-email").value.trim()
      }, $("login-status"), function () {
        setStatus($("login-status"), "ok", "Lien magique envoye. Verifiez votre email.");
      });
    });
    ...
    $("forgot-form").addEventListener("submit", function (event) {
      event.preventDefault();
      handleJsonForm("/api/client/forgot-password", {
        email: $("forgot-email").value.trim()
      }, $("forgot-status"), function () {
        setStatus($("forgot-status"), "ok", "Si le compte existe, un lien de reinitialisation a ete envoye.");
      });
    });
