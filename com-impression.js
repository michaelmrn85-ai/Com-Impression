...
"comservices":[
{id:'impression-doc',nom:'Impression de document',prix:'dès 0,05 €/page',opts:{'Grammage':['80g','90g','100g','120g','160g'],'Format':['A4','A5'],'Couleur':['Noir & Blanc','Couleur'],'Impression':['Recto','Recto-Verso']},T:{'80g|N&B|Recto':[0.10,0.08,0.06,0.05],'80g|N&B|RV':[0.18,0.14,0.10,0.09],'80g|Col|Recto':[0.10,0.08,0.06,0.05],'80g|Col|RV':[0.18,0.14,0.10,0.09],'90g|N&B|Recto':[0.12,0.10,0.08,0.07],'90g|N&B|RV':[0.22,0.18,0.14,0.12],'90g|Col|Recto':[0.12,0.10,0.08,0.07],'90g|Col|RV':[0.22,0.18,0.14,0.12],'100g|N&B|Recto':[0.14,0.12,0.10,0.08],'100g|N&B|RV':[0.26,0.22,0.18,0.15],'100g|Col|Recto':[0.14,0.12,0.10,0.08],'100g|Col|RV':[0.26,0.22,0.18,0.15],'120g|N&B|Recto':[0.18,0.15,0.12,0.10],'120g|N&B|RV':[0.34,0.28,0.22,0.18],'120g|Col|Recto':[0.18,0.15,0.12,0.10],'120g|Col|RV':[0.34,0.28,0.22,0.18],'160g|N&B|Recto':[0.25,0.21,0.17,0.14],'160g|N&B|RV':[0.48,0.40,0.32,0.26],'160g|Col|Recto':[0.25,0.21,0.17,0.14],'160g|Col|RV':[0.48,0.40,0.32,0.26]},Q:[1,11,50,1000],dims:false,unite:'page'},
...
function calcPrix(prod,sels,qte,larg,haut){
  ...
  if(prod.id==='impression-doc'){
    ...
    var gm=(sels['Grammage']||'80g');
    var gr=gm.indexOf('160')>-1?'160g':gm.indexOf('120')>-1?'120g':gm.indexOf('100')>-1?'100g':gm.indexOf('90')>-1?'90g':'80g';
...
async function chargerSiteConfig(){
  ...
  try {
    ...
    var bannerTitle = document.getElementById('home-banner-title');
    if (bannerTitle) bannerTitle.textContent = (config.heroLine1 || (heroLine1 && heroLine1.textContent) || bannerTitle.textContent);
    ...
    var bannerSubtitle = document.getElementById('home-banner-subtitle');
    if (bannerSubtitle) bannerSubtitle.textContent = (config.heroLine2 || (heroLine2 && heroLine2.textContent) || bannerSubtitle.textContent);
    ...
    if (heroImageWrap && heroImage) {
      var defaultHeroImage = heroImage.getAttribute('data-default-src') || heroImage.getAttribute('src') || '';
      if (config.heroImage) {
        heroImage.src = config.heroImage;
        heroImageWrap.classList.add('show');
        if (homeBanner) homeBanner.classList.add('has-image');
        ...
        if (homeBanner) homeBanner.classList.remove('has-image');
...
function renderAvisPublic(list){
  ...
  if(!avis.length){
    grid.innerHTML='<div class="avis-card avis-card-empty">Aucun avis publie pour le moment.</div>';
    return;
  }
  grid.innerHTML=avis.slice(0,6).map(function(a){
    var stars='★★★★★'.slice(0,Math.max(1,Math.min(5,parseInt(a.note,10)||5)));
    var meta=[a.prenom||'Client',a.ville||''].filter(Boolean).join(' - ');
    return '<article class="avis-card">'
      +'<div class="avis-stars">'+stars+'</div>'
      +'<p class="avis-text">« '+escHtml(a.texte||'')+' »</p>'
      +'<div class="avis-name">'+escHtml(meta||'Client COM Impression')+'</div>'
      +'<div class="avis-product">'+escHtml(a.produit||'Avis client')+'</div>'
      +'</article>';
  }).join('');
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  var rdvContact=document.getElementById('rdv-contact-btn'); if(rdvContact) rdvContact.addEventListener('click',function(e){e.preventDefault();openIfExists('m-contact');});
  ...
  var homeAccountLink=document.getElementById('home-account-link');
  if(homeAccountLink) homeAccountLink.addEventListener('click',function(e){
    e.preventDefault();
    if(btnCompte) btnCompte.click();
  });
